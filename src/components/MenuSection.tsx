      ...
      {items.map((item) => (
        <div key={item.name} className="flex justify-between items-baseline gap-4 group">
          <div className="flex-1">
            <span className="font-body font-medium text-foreground group-hover:text-primary transition-colors">{item.name}</span>
...
const MenuSection = () => {
  ...
  return (
    <section id="menu" ref={ref} className="section-padding bg-secondary">
      <div className={`max-w-5xl mx-auto transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
        <p className="text-primary font-body text-sm tracking-[0.3em] uppercase text-center mb-3">What We Serve</p>
        <h2 className="font-display text-4xl md:text-5xl font-bold text-center mb-16">Our Menu</h2>

        <div className="grid md:grid-cols-2 gap-x-16">
          <div>
            <MenuCategory title="Classic Bagels" items={[
            ...
            <MenuCategory title="Special Bagels" items={[
            ...
            <MenuCategory title="Sourdough Bagels" items={[
              { name: "Plain / Sesame / Lye / Everything", price: "€2.00" },
            ]} />
            <MenuCategory title="Dips" items={[
              { name: "Jomi Herb Cream Cheese", price: "€2.50" },
            ]} />
          </div>
          <div>
            <MenuCategory title="Hot Drinks" items={[
