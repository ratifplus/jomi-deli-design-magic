    ...
    >
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between h-20">
        ...
        <div className="hidden md:flex items-center gap-8">
          {navLinks.map((l) => (
            <button key={l.href} onClick={() => handleNav(l.href)} className="gold-underline text-sm font-body font-medium tracking-wide text-muted-foreground hover:text-foreground transition-colors">
              {l.label}
            </button>
          ))}
          <a
          ...
          <div className="px-6 py-6 flex flex-col gap-4">
            {navLinks.map((l) => (
              <button
                ...
                className="text-left text-lg font-body text-muted-foreground hover:text-foreground transition-colors"
              >
                {l.label}
              </button>
            ))}
